...
<?php 
     if(isset($_POST['btn1'])) 
     {
        ...
        $query="INSERT INTO `appointment`(`appfor`, `docid`, `patientname`,`emailid`, `appdate`, `apptime`, `price`, `remark`, `status`) VALUES ('$appfor','$docid','$patientname','$emailid','$appdate','$apptime','$price','$remark','Pending')";
        ...
        if ($result) {
            // Fetch doctor name
            $query2 = "SELECT docname FROM dentist WHERE docid = '$docid'";
            $doctorResult = $dc->getrecord($query2);
            $docname = $doctorResult ? $doctorResult["docname"] : "Unknown";
        
            $queryCheck = "SELECT `status` FROM `appointment` WHERE `patientname`='$patientname' AND `emailid`='$emailid' AND `appdate`='$appdate'";
            $statusResult = $dc->getrecord($queryCheck);
            ...
            if ($statusResult && $statusResult['status'] === 'Approved') {
                echo "<script>
                        Swal.fire({
                            ...
                            text: '✅ Your appointment with Dr. $docname on $appdate at $apptime is confirmed.',
                            confirmButtonColor: '#28a745',
                            confirmButtonText: 'Proceed for Payment'
                        }).then((result) => {
                            if (result.isConfirmed) {
                                window.location.href='appontment_confirmation.php';
                            }
                        });
                      </script>";
            }
